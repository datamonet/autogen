...
def init_db_samples(dbmanager: Any):
    ...
    gpt_4_model = Model(
        model="gpt-4-1106-preview", description="OpenAI GPT-4 model", user_id="curator@takin.ai", api_type="open_ai"
    )
    ...
    local_assistant = Agent(
        user_id="curator@takin.ai",      public=True, type=AgentType.assistant, config=local_assistant_config.model_dump(mode="json")
    )
    ...
    travel_groupchat_agent = Agent(
        user_id="curator@takin.ai",       public=True,type=AgentType.groupchat, config=travel_groupchat_config.model_dump(mode="json")
    )
    ...
    user_proxy = Agent(
        user_id="curator@takin.ai",      public=True, type=AgentType.userproxy, config=user_proxy_config.model_dump(mode="json")
    )
    ...
    default_assistant = Agent(
        user_id="curator@takin.ai",      public=True, type=AgentType.assistant, config=default_assistant_config.model_dump(mode="json")
    )
