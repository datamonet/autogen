...
#         user_id="faye_1225@163.com", type=AgentType.assistant, config=default_assistant_config.model_dump(mode="json")
#     )
#
#     # workflows
...
def init_db_samples(dbmanager: Any):
    ...
    models_to_add = [
        ...
        Model(
            ...
            description="Azure OpenAI model",
            api_type="azure",
            base_url="https://api.yourazureendpoint.com/v1",
        ),
        Model(
            model="zephyr",
            description="Local Huggingface Zephyr model via vLLM, LMStudio or Ollama",
    ...
    with Session(dbmanager.engine) as session:
        existing_models = {model.model for model in session.query(Model).all()}

        models_to_add_filtered = [model for model in models_to_add if model.model not in existing_models]

        if models_to_add_filtered:
            session.add_all(models_to_add_filtered)
            session.commit()
            logger.info("Successfully initialized database with Default and Travel Planning Workflows")
        else:
            logger.info("Models already exist in the database; no new models added.")
