...
def init_db_samples(dbmanager: Any, user_id: str = "curator@takin.ai"):
    ...
    with Session(dbmanager.engine) as session:
        ...
        existing_workflows = session.query(Workflow).first()
        if existing_agents or existing_workflows:
            logger.info("Database already contains agents or workflows, skipping initialization")
            return
    ...
    # models
    gpt_4o_model = Model(
        model="gpt-4o", description="OpenAI GPT-4o model", user_id=user_id, api_type="open_ai"
    )
    gpt_4o_mini_model = Model(
        model="gpt-4o-mini", description="OpenAI GPT-4o-mini model", user_id=user_id, api_type="open_ai"
    )
    ...
    local_assistant = Agent(
        user_id=user_id, public=True, type=AgentType.assistant,
    ...
    travel_groupchat_agent = Agent(
        user_id=user_id, public=True, type=AgentType.groupchat,
    ...
    user_proxy = Agent(
        user_id=user_id, public=True, type=AgentType.userproxy,
    ...
    with Session(dbmanager.engine) as session:
        ...
        dbmanager.link(link_type="agent_model", primary_id=default_assistant.id, secondary_id=gpt_4o_mini_model.id)
